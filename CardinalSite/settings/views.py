...
def import_data(request):
    if request.method == "POST" and request.FILES.get("file"):
        ...
        if file.name.endswith(".sqlite3"):
            ...
            messages.error(
                request,
                "Invalid file format. Please select a valid SQLite database file.",
            )
    else:
...
def delete_database(request):
    if request.method == "POST":
        ...
        if confirm == "confirmed":
            ...
            messages.success(
                request, "The database has been cleared except superusers."
            )
            return redirect("settings:settings")
        else:
            messages.error(
                request,
                "Confirmation was not provided. Database deletion was canceled.",
            )
            return redirect("settings:settings")
